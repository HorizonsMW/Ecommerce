...
document.addEventListener('DOMContentLoaded', function () {
        ...
        filterResetButton.addEventListener('click', function() {
            ...
            if (sortSelect) {
                ...
                products.sort((a, b) => {
                    switch (currentSort) {
                        case 'price-low':
                            return parseFloat(a.querySelector('.product-price').textContent.match(/\d+/)[0]) -
                                parseFloat(b.querySelector('.product-price').textContent.match(/\d+/)[0]);
                        case 'price-high':
                            return parseFloat(b.querySelector('.product-price').textContent.match(/\d+/)[0]) -
                                parseFloat(a.querySelector('.product-price').textContent.match(/\d+/)[0]);
                        case 'name-asc':
                            return a.querySelector('.product-title').textContent.trim().localeCompare(
                                b.querySelector('.product-title').textContent.trim()
                            );
                        case 'name-desc':
                            return b.querySelector('.product-title').textContent.trim().localeCompare(
    ...
    if (filtersColumn) {
        document.addEventListener('click', function(event) {
            if (!filtersColumn.contains(event.target) && !filterToggle.contains(event.target)) {
                filtersColumn.classList.remove('active');
            }
        });
    }
    ...
    if (filterOkButton) {
        filterOkButton.addEventListener('click', function() {
            ...
            const filteredProducts = allProducts.filter(product => {
                ...
                // Check availability
                const availabilityMatch = selectedAvailability.length === 0 || selectedAvailability.some(avail => {
                    if (avail === 'in-stock') return productQuantity > 0;
                    if (avail === 'low-stock') return productQuantity > 0 && productQuantity <= 10;
                    return true;
                });

                return categoryMatch && priceMatch && brandMatch && colorMatch && availabilityMatch;
            });
